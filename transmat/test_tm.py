...
def usage(errno=0):
    ...
    -e --encoding <string>: type of encoding. Default is utf8.
...
def main(sys_argv):
    ...
    try:
        opts, argv = getopt.getopt(sys_argv[1:], "hoe:c:",
                                   ["help", "output=", "correction=", "encoding"])
    ...
    if not additional:
        source_sp = Space.build(source_file, source_words, encoding=source_encoding)
    else:
        ...
        source_sp = Space.build(source_file, source_words.union(set(lexicon)), encoding=source_encoding)
    
    source_sp.normalize()

    print("Reading: %s" % target_file)
    target_sp = Space.build(target_file, encoding=target_encoding)
    target_sp.normalize()
